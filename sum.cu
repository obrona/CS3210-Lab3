...
__global__ void reduceAtomicShared(const unsigned long long* __restrict input, unsigned long long N)
{
    ...
    // TODO: Your code here
    __shared__ unsigned long long store;
    if (threadIdx.x == 0) store = 0;
    __syncthreads();

    if (id < N) atomicAdd(&store, input[id]);
    __syncthreads();
    if (!threadIdx.x && id < N) atomicAdd(&dResult, store);
...
__global__ void reduceShared(const unsigned long long* __restrict input, unsigned long long N)
{
    ...
    for (int i = 2; i <= BS; i <<= 1) {
        if (!(threadIdx.x & (i - 1))) {
            data[threadIdx.x] += data[threadIdx.x + (i >> 1)];
        }
        __syncthreads();
    }

    // only threadIdx.x == 0 adds the result to global result
    if (threadIdx.x == 0) atomicAdd(&dResult, data[0]);
